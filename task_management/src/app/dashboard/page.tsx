...
interface Task {
  ...
  status: 'Pending' | 'In Progress' | 'Completed'; // Corrected status values
  assignee?: { email: string; fullName: string } | null;
  creator: { email: string; fullName: string }; // Creator should also have fullName
  createdAt: string;
}
...
const getTasks = async (token: string, filters?: { title?: string; status?: string; priority?: string; dueDate?: string; assigneeEmail?: string }) => {
  try {
    ...
    return data as Task[]; // Type assertion to Task[]
  } catch (error) {
  ...
  const handleCreate = async (newTaskData: Omit<Task, '_id' | 'createdAt' | 'creator'>) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tasks`, {
        ...
        body: JSON.stringify({ ...newTaskData, creatorEmail: userInfo.email }), // Use creatorEmail
      });
        ...
        <aside className={`bg-gradient-to-b from-teal-600 to-blue-700 text-white shadow-xl transition-all duration-300 overflow-hidden ${isSidebarOpen ? 'w-64 p-6' : 'w-16 p-2'}`}>
          ...
          <ul className="space-y-4">
            {['dashboard', 'tasks', 'search'].map((section) => (
              <li
                key={section}
                className={`cursor-pointer p-3 rounded-lg transition-all duration-200 hover:bg-teal-500 flex items-center ${activeSection === section ? 'bg-teal-500' : ''}`}
                onClick={() => handleSectionClick(section)}
              >
                {section === 'dashboard' && '📊'}
                {section === 'tasks' && '📝'}
                {section === 'search' && '🔍'}
                <span className={`${isSidebarOpen ? 'ml-4' : 'hidden'}`}>{section.charAt(0).toUpperCase() + section.slice(1).replace('-', ' ')}</span>
              </li>
            ))}
          </ul>
        </aside>
        ...
        <main className="flex-1 p-6 bg-gray-50 overflow-auto">
          {activeSection === 'dashboard' && (
            <>
              ...
              <h2 className="text-xl font-semibold mt-4 mb-2">Assigned to You</h2>
              <TaskList tasks={tasks.filter((task) =>  task.assignee?. fullName  === userInfo.name)} onUpdate={handleUpdate} onDelete={handleDelete} />
              <h2 className="text-xl font-semibold mt-6 mb-2">Created by You</h2>
              <TaskList tasks={tasks.filter((t) => t.creator.email === userInfo.email)} onUpdate={handleUpdate} onDelete={handleDelete} />
              <h2 className="text-xl font-semibold mt-6 mb-2">Overdue Tasks</h2>
